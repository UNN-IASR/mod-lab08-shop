...
int main() {
    ...
    Market market(cashiersNum, cashiersSpeed, customerRate, customerCart, queueSize);
    market.Work(100);
    
    std::cout << "Reject Probability: " << market.statRejected / clientNum
            << "\nRelative Throughput: " << market.statAccepted / clientNum
            << "\nAbsolute Throughput: " << market.statAccepted * customerRate / clientNum
            << "\nAverage Customers Count In Queue: " << market.statQueueLen / clientNum
            << "\nAverage Customer Time In Store: " << market.statCustTime / 1000 / market.statAccepted
            << std::endl;
}