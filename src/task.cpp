...
customer::customer(int avgItems, int i) {
    tQueue = 0;
    tService = 0;
...
void customer::outCashbox(int time) {
    tService = time;
}
...
Shop::Shop(int countcashbox, int requestrate, int servicespeed, int avgitems, int maxqueue) {
    ...
    queueClients = std::deque<customer*>();
...
void Shop::Work() {
    ...
    for (int i = 0; i < requestRate; i++) {
        clients.push_back(customer(avgItems, i + 1));
    }
    ...
    while (countClients < requestRate) {
        ...
        if (queueClients.size() < maxQueue)
        {
            ...
            clients.at(countClients).IN();
            m.unlock();
        }
        else {
            rejectedClients++;
            std::cout << "Клиент №" << clients.at(countClients).id << "Не обслужен" << std::endl;
    ...
    std::cout << "Обслуженные клиенты: " << std::to_string(servedClients) << std::endl;
    std::cout << "Отклоненные клиенты: " << std::to_string(rejectedClients) << std::endl;
    std::cout << "Средняя очередь : " << std::to_string(avgQueueLen) << std::endl;
    std::cout << "Среднее время клиента: " << std::to_string(aTimeinqueue / 1000) << " i " << std::to_string(aTimeincashbox / 1000) << std::endl;
    std::cout << "Среднее время работы кассы: " << std::to_string(aWork / 1000) << std::endl;
    std::cout << "Среднее время остановки кассы: " << std::to_string(aWait / 1000) << std::endl;
    ...
    std::cout << "Отказ: " << std::to_string(P) << std::endl;
    std::cout << " Относительная пропускная способность: " << std::to_string(Q) << std::endl;
    std::cout << "Абсолютная пропускная способность: " << std::to_string(A) << std::endl;
}